...
```text
Text -> multi-scale CNN -> BiGRU -----\
Audio -> STFT -> CNN -----------------> gated multimodal state -> typed heads
IMU  -> 1D CNN -> BiGRU -------------/
Question -> shared encoder ------------/
```

The model supports NOUL, SCORE, and CHOICE decisions, direct low-rank adapters, local audio DSP features, accelerometer sequences, synthetic signal augmentation with a GAN, held-out temperature calibration, teacher distillation, and a model registry/promotion workflow.
...
`DECISION_BACKEND=auto` tries the owned checkpoint first, then Laya, optional Jev, and deterministic fallback. A missing or unvalidated owned checkpoint therefore does not break the local inbox application.

See `docs/OWNED_MODEL.md` and `docs/RESEARCH_TRACK.md` for the architecture, calibration contract, distillation workflow, dataset strategy, and evaluation gates.
